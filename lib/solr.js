...
Client.prototype.search = function(query,callback){
   ...
   var parameters = query.build ? query.build() : query,
      requestHandler = query.requestHandler ? query.requestHandler : this.SELECT_HANDLER;

   this.options.fullPath = [this.options.path,this.options.core,  requestHandler + '?' + parameters + '&wt=json']
                              .filter(function(element){
                                 return element;    
                              })
                              .join('/'); ;
