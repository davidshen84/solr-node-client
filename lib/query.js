...
Query.prototype.requestHandler = function(name){
  var self = this;

  self.requestHandler = name;
...
Query.prototype.tvrh = function (options) {
   var self = this,
      optArr = [ 'tf', 'df', 'positions', 'offset', 'tf_idf', 'all', 'fl', 'docids' ],
      i,
      opt,
      val;
   ...
   for (i = 0; i < optArr.length; i++) {
      ...
      if (val) {
         self.parameters.push('tv.' + opt + '=' + val);
      }
   }
